package Celeste.spring.mvc.utils;

import Celeste.spring.mvc.vo.Pds;
...
public class FileUpDownUtil {
    ...
    // 자바 웹 프로그래밍 파일 업로드 지원
    // 1. oreilly사의 '자바 서블릿 프로그래밍' 도서의 예제 - cos.jar
    // 2. apache commons project - FileUpload (추천!) => commons.apache.org
    // 3. Servlet 3.x부터 파일 업로드 내장 - MultipartFile

    // 파일 업로드 경로 지정
    private String uploadPath = "/Users/celeste/Documents/pdsupload/";
                ...
                while (params.hasNext()) {
                    try {
                        ...
                        if (item.isFormField()) { // 텍스트 데이터라면
                            ...
                            // 업로드한 파일을 시스템에 저장할 때는 파일명+uuid 로 설정
                            item.write(f);  // 지정한 경로에 파일 저장

                            String name = item.getFieldName();
                            // 업로드한 파일 정보를 디비에 저장할 때는 원본 파일명 그대로 설정
    ...
    public void procDownload(String fname, String uuid, HttpServletResponse res) {
        ...
        try {
            ...
            // 다운로드할 파일의 실제 위치를 파악하고, 파일의 내용을 stream으로 미리 읽어둠
            try {
